 ...
 *   e.g. node scripts/merge-drive-links.js ~/Downloads/DriveManifest.csv ./data
 */
...
const DATA_DIR = process.argv[3] || path.join(__dirname, '..', 'data');
...
for (const s of sources) {
  if (s.kind === 'rich') {
    ...
    for (const d of docs) {
      ...
      if (hit) { d.driveUrl = resolveUrl(hit); matched++; }
...
function mergePlainNode(node, relPath, driveFolderName, manifest, stats) {
  if (node.type === 'file') {
    ...
    const hit = manifest.get(joinPath);
    if (hit) { node.driveUrl = resolveUrl(hit); stats.matched++; }
    else { delete node.driveUrl; stats.missing++; }
    return;
  }
